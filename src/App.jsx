...
function WalletView({ balance, notify, refresh }) {
  ...
  const qrUrl = payment?.bank_code && payment?.bank_account_number ? `https://img.vietqr.io/image/${encodeURIComponent(payment.bank_code)}-${encodeURIComponent(payment.bank_account_number)}-compact2.png?addInfo=${encodeURIComponent(payment.transfer_prefix || 'QMSTORE')}&accountName=${encodeURIComponent(payment.bank_account_name || '')}` : null
