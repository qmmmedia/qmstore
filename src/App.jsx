...
function ServiceCategoryBrowser({ service, close, onOrder }) {
  ...
  const current = parentId ? items.find(item => item.id === parentId) : null
  const visible = items.filter(item => (item.parent_id || null) === parentId).sort((a, b) => a.sort_order - b.sort_order)
  const hasChildren = item => items.some(child => child.parent_id === item.id)
  return <section className="panel category-browser"><div className="editor-heading"><div><span className="eyebrow">DANH MỤC DỊCH VỤ</span><h2>{current?.title || service.name}</h2><p>{current ? 'Chọn hạng mục bên dưới để xem chi tiết.' : 'Chọn một danh mục để tiếp tục.'}</p></div><button className="square" type="button" onClick={close}><X size={18}/></button></div>{parentId && <button className="back-category" onClick={() => { const parent = items.find(item => item.id === parentId); setParentId(parent?.parent_id || null) }}>← Quay lại</button>}<div className="category-list">{visible.length ? visible.map(item => <button className="category-node" key={item.id} onClick={() => hasChildren(item) ? setParentId(item.id) : onOrder()}><span><b>{item.title}</b>{item.description && <small>{item.description}</small>}</span><ChevronRight size={18}/></button>) : <p className="empty-note">Chưa có danh mục con. Admin có thể thêm trong Quản lý dịch vụ.</p>}</div></section>
}

function Order({ services, session, balance, refresh, notify }) { const [serviceId, setServiceId] = useState(services[0]?.id); const [quantity, setQuantity] = useState(1); const [coupon, setCoupon] = useState(''); const selected = useMemo(() => services.find(s => s.id === serviceId) || services[0], [services, serviceId]); if (!selected) return <section className="page"><h1>Tạo đơn dịch vụ</h1><div className="panel empty-services"><Package size={26}/><h2>Chưa có dịch vụ để đặt</h2><p>Vui lòng thêm dịch vụ mới trong khu vực Quản lý dịch vụ trước.</p></div></section>; const discount = coupon.toUpperCase() === 'QM10' ? Math.min(50000, Math.round(selected.price * quantity * .1)) : 0; const total = selected.price * quantity - discount
  ...
  return <section className="page">
    ...
    {tab === 'services' ? <>
      <form className="panel admin-form" onSubmit={createService}>
        ...
        <div className="two-fields"><Field label="Tên dịch vụ" value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })} required/><Field label="Danh mục" value={draft.category} onChange={e => setDraft({ ...draft, category: e.target.value })} required/></div>
...
function ServiceItemsEditor({ service, itemDraft, setItemDraft, parentId, setParentId, addItem, deleteItem, close }) {
  ...
  return <section className="panel service-items-editor"><div className="editor-heading"><div><h2>Danh mục nhiều cấp</h2><p>{service.name} → danh mục con → mục triển khai</p></div><button className="square" type="button" onClick={close}><X size={18}/></button></div><form onSubmit={addItem}><div className="two-fields"><label className="field"><span>Thuộc danh mục</span><select value={parentId} onChange={e => setParentId(e.target.value)}><option value="">Dịch vụ chính — cấp đầu</option>{options.map(option => <option key={option.id} value={option.id}>{'— '.repeat(option.depth)}{option.title}</option>)}</select></label><Field label="Tên danh mục / mục con" value={itemDraft.title} onChange={e => setItemDraft({ ...itemDraft, title: e.target.value })} placeholder="Ví dụ: Facebook" required/></div><Field label="Mô tả ngắn" value={itemDraft.description} onChange={e => setItemDraft({ ...itemDraft, description: e.target.value })} placeholder="Ví dụ: Lập kế hoạch nội dung và quảng cáo"/><button className="primary-btn">Thêm vào cấp đã chọn <ChevronRight size={17}/></button></form><div className="item-list">{service.items.length ? <ServiceItemTree items={service.items} parentId={null} depth={0} setParentId={setParentId} deleteItem={deleteItem}/> : <p className="empty-note">Chưa có danh mục nào. Hãy thêm mục cấp đầu tiên.</p>}</div></section>
}
