...
function Services({ services, setScreen }) {
  ...
  return <section className="page"><h1>Dịch vụ QM STORE</h1><p className="sub">Giải pháp digital minh bạch, triển khai theo yêu cầu của bạn.</p><div className="service-grid all-services">{services.map(s => <ServiceCard key={s.id} service={s} onOrder={() => setScreen('order')} onBrowse={() => setSelectedService(s)}/>)}</div>{selectedService && <ServiceCategoryBrowser service={selectedService} close={() => setSelectedService(null)} onOrder={() => { setSelectedService(null); setScreen('order') }}/>}</section>
}
