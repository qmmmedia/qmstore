...
import './payment-tools.css'
import { BarChart3, Bell, ChevronRight, CreditCard, FileText, Gift, LayoutDashboard, LogOut, Menu, Moon, Package, Plus, Search, Settings, ShoppingCart, Sun, UserRound, Wallet, X } from 'lucide-react'
...
async function openTopupProof(path, notify) { const { data, error } = await supabase.storage.from('topup-proofs').createSignedUrl(path, 300); if (error || !data?.signedUrl) return notify(error?.message || 'Không thể mở biên lai.'); window.open(data.signedUrl, '_blank', 'noopener,noreferrer') }
...
function WalletView({ balance, notify, refresh }) {
  const [requests, setRequests] = useState([]); const [transactions, setTransactions] = useState([]); const [payment, setPayment] = useState(null); const [showForm, setShowForm] = useState(false); const [loading, setLoading] = useState(false)
  async function loadWallet() { if (!supabase) return; const [{ data: topups }, { data: history }, { data: settings }] = await Promise.all([supabase.from('wallet_topup_requests').select('*').order('created_at', { ascending: false }).limit(8), supabase.from('wallet_transactions').select('*').order('created_at', { ascending: false }).limit(12), supabase.from('store_settings').select('*').eq('id', 1).maybeSingle()]); if (topups) setRequests(topups); if (history) setTransactions(history); if (settings) setPayment(settings) }
  useEffect(() => { loadWallet() }, [])
  async function submit(event) { event.preventDefault(); if (!supabase) return notify('Hãy kết nối Supabase trước.'); const form = new FormData(event.currentTarget); const amount = Number(form.get('amount')); if (!amount || amount < 10000) return notify('Số tiền nạp tối thiểu là 10.000đ.'); const { data: auth } = await supabase.auth.getUser(); const userId = auth.user?.id; if (!userId) return notify('Phiên đăng nhập đã hết hạn.'); const proof = form.get('proof'); let proofPath = null; if (proof instanceof File && proof.size) { if (!proof.type.startsWith('image/') || proof.size > 5 * 1024 * 1024) return notify('Biên lai phải là ảnh và không quá 5MB.'); const safeName = proof.name.replace(/[^a-zA-Z0-9._-]/g, '-'); proofPath = `${userId}/${Date.now()}-${safeName}`; setLoading(true); const { error: uploadError } = await supabase.storage.from('topup-proofs').upload(proofPath, proof, { contentType: proof.type }); if (uploadError) { setLoading(false); return notify(uploadError.message) } } setLoading(true); const { error } = await supabase.from('wallet_topup_requests').insert({ user_id: userId, amount, payment_reference: String(form.get('reference')).trim(), note: String(form.get('note')).trim() || null, proof_path: proofPath }); setLoading(false); if (error) return notify(error.message); event.currentTarget.reset(); setShowForm(false); await loadWallet(); await refresh(); notify('Đã gửi yêu cầu nạp tiền. Admin sẽ kiểm tra và duyệt.'); }
  const qrUrl = payment?.bank_code && payment?.bank_account_number ? `https://img.vietqr.io/image/${encodeURIComponent(payment.bank_code)}-${encodeURIComponent(payment.bank_account_number)}-compact2.png?amount=&addInfo=${encodeURIComponent(payment.transfer_prefix || 'QMSTORE')}&accountName=${encodeURIComponent(payment.bank_account_name || '')}` : null
  return <section className="page"><h1>Ví của tôi</h1><p className="sub">Quản lý số dư, nạp tiền và theo dõi các giao dịch đã phát sinh.</p><div className="wallet-show"><span>QM WALLET</span><strong>{money(balance)}</strong><p>Số dư khả dụng</p><button onClick={() => setShowForm(value => !value)}>Nạp tiền <Plus size={16}/></button></div>{showForm && <form className="panel profile-form topup-form" onSubmit={submit}><h2><CreditCard size={20}/> Gửi yêu cầu nạp tiền</h2>{qrUrl ? <div className="payment-qr"><img src={qrUrl} alt="Mã QR chuyển khoản"/><div><b>{payment.bank_code} · {payment.bank_account_number}</b><p>{payment.bank_account_name || 'Chủ tài khoản chưa cập nhật'}</p><small>Nội dung chuyển khoản: {payment.transfer_prefix || 'QMSTORE'}</small></div></div> : <p className="empty-note">Admin chưa cấu hình QR thanh toán. Bạn vẫn có thể gửi yêu cầu nạp thủ công.</p>}<Field label="Số tiền (VNĐ)" name="amount" type="number" min="10000" placeholder="Ví dụ: 100000" required/><Field label="Mã giao dịch / nội dung chuyển khoản" name="reference" placeholder={`Ví dụ: ${payment?.transfer_prefix || 'QMSTORE'} 123456`} required/><label className="field"><span>Ảnh biên lai (tối đa 5MB)</span><input name="proof" type="file" accept="image/*"/></label><label className="field"><span>Ghi chú</span><textarea name="note" placeholder="Thông tin bổ sung nếu cần"/></label><button className="primary-btn" disabled={loading}>{loading ? 'Đang gửi...' : 'Gửi yêu cầu nạp tiền'} <ChevronRight size={17}/></button></form>}<SectionTitle title="Yêu cầu nạp tiền"/><div className="table-panel"><table><thead><tr><th>Yêu cầu nạp</th><th>Mã giao dịch</th><th>Biên lai</th><th>Trạng thái</th><th>Ngày tạo</th></tr></thead><tbody>{requests.length ? requests.map(request => <tr key={request.id}><td>{money(request.amount)}</td><td>{request.payment_reference}</td><td>{request.proof_path ? <button className="ghost-btn" onClick={() => openTopupProof(request.proof_path, notify)}>Xem ảnh</button> : '—'}</td><td><span className={'status ' + (request.status === 'approved' ? 'completed' : request.status === 'rejected' ? 'failed' : 'pending')}>{request.status === 'approved' ? 'Đã duyệt' : request.status === 'rejected' ? 'Từ chối' : 'Chờ duyệt'}</span></td><td>{new Date(request.created_at).toLocaleDateString('vi-VN')}</td></tr>) : <tr><td colSpan="5" className="empty-cell">Chưa có yêu cầu nạp tiền nào.</td></tr>}</tbody></table></div><SectionTitle title="Lịch sử giao dịch"/><div className="table-panel"><table><thead><tr><th>Loại giao dịch</th><th>Mô tả</th><th>Số tiền</th><th>Thời gian</th></tr></thead><tbody>{transactions.length ? transactions.map(transaction => <tr key={transaction.id}><td>{transaction.type === 'topup' ? 'Nạp tiền' : transaction.type === 'payment' ? 'Thanh toán đơn' : transaction.type === 'refund' ? 'Hoàn tiền' : 'Điều chỉnh'}</td><td>{transaction.description || 'Giao dịch QM STORE'}</td><td className={Number(transaction.amount) >= 0 ? 'green' : ''}>{Number(transaction.amount) >= 0 ? '+' : ''}{money(transaction.amount)}</td><td>{new Date(transaction.created_at).toLocaleString('vi-VN')}</td></tr>) : <tr><td colSpan="4" className="empty-cell">Chưa có giao dịch nào.</td></tr>}</tbody></table></div><div className="panel info-card"><Gift/><div><b>Ưu đãi QM10</b><p>Giảm 10%, tối đa 50.000đ cho đơn từ 100.000đ.</p></div></div></section>
}
...
function Admin({ services, refresh, notify, initialTab }) {
  ...
  if (tab === 'payment') return <AdminPaymentSettings notify={notify}/>
  if (tab === 'reports') return <AdminReports orders={orders} topups={topups}/>
...
function AdminPaymentSettings({ notify }) {
  const [settings, setSettings] = useState({ id: 1, bank_code: '', bank_account_number: '', bank_account_name: '', transfer_prefix: 'QMSTORE' })
  const [loading, setLoading] = useState(true)
  useEffect(() => { async function load() { const { data } = await supabase.from('store_settings').select('*').eq('id', 1).maybeSingle(); if (data) setSettings(data); setLoading(false) } load() }, [])
  function change(key, value) { setSettings(current => ({ ...current, [key]: value })) }
  async function save(event) { event.preventDefault(); const { data: auth } = await supabase.auth.getUser(); const payload = { ...settings, id: 1, bank_code: settings.bank_code.trim().toUpperCase(), bank_account_number: settings.bank_account_number.trim(), bank_account_name: settings.bank_account_name.trim(), transfer_prefix: settings.transfer_prefix.trim() || 'QMSTORE', updated_at: new Date().toISOString(), updated_by: auth.user?.id || null }; const { error } = await supabase.from('store_settings').upsert(payload); if (error) return notify(error.message); setSettings(payload); notify('Đã lưu cấu hình thanh toán. QR sẽ hiện ngay cho khách hàng.') }
  const qrUrl = settings.bank_code && settings.bank_account_number ? `https://img.vietqr.io/image/${encodeURIComponent(settings.bank_code)}-${encodeURIComponent(settings.bank_account_number)}-compact2.png?amount=&addInfo=${encodeURIComponent(settings.transfer_prefix || 'QMSTORE')}&accountName=${encodeURIComponent(settings.bank_account_name || '')}` : null
  return <section className="page"><h1>Cấu hình thanh toán</h1><p className="sub">Nhập thông tin tài khoản ngân hàng để khách nhận QR chuyển khoản khi nạp tiền.</p><form className="panel profile-form payment-settings" onSubmit={save}><h2><CreditCard size={20}/> Tài khoản nhận tiền</h2><Field label="Mã ngân hàng VietQR" value={settings.bank_code || ''} onChange={event => change('bank_code', event.target.value)} placeholder="Ví dụ: MB, VCB, ACB" required/><Field label="Số tài khoản" value={settings.bank_account_number || ''} onChange={event => change('bank_account_number', event.target.value)} placeholder="Nhập số tài khoản" required/><Field label="Tên chủ tài khoản" value={settings.bank_account_name || ''} onChange={event => change('bank_account_name', event.target.value)} placeholder="Ví dụ: NGUYEN VAN A" required/><Field label="Tiền tố nội dung chuyển khoản" value={settings.transfer_prefix || ''} onChange={event => change('transfer_prefix', event.target.value)} placeholder="QMSTORE" required/><button className="primary-btn" disabled={loading}>{loading ? 'Đang tải...' : 'Lưu cấu hình thanh toán'}</button></form>{qrUrl && <section className="panel qr-preview"><h2>QR xem trước</h2><img src={qrUrl} alt="QR thanh toán xem trước"/><p>Khách sẽ quét mã này tại Ví của tôi → Nạp tiền.</p></section>}</section>
}
function AdminReports({ orders, topups }) {
  const paidOrders = orders.filter(order => !['failed', 'refunded'].includes(order.status)); const revenue = paidOrders.reduce((sum, order) => sum + Number(order.total_amount || 0), 0); const approvedTopups = topups.filter(request => request.status === 'approved'); const topupTotal = approvedTopups.reduce((sum, request) => sum + Number(request.amount || 0), 0); const completed = orders.filter(order => order.status === 'completed').length
  function exportOrders() { const quote = value => `"${String(value ?? '').replaceAll('"', '""')}"`; const rows = [['Mã đơn', 'Khách hàng', 'Email', 'Dịch vụ', 'Gói', 'Tổng tiền', 'Trạng thái', 'Link/Ghi chú', 'Ngày tạo'], ...orders.map(order => [order.id, order.profiles?.full_name || '', order.profiles?.email || '', order.services?.name || '', order.service_items?.title || '', order.total_amount, statusLabel(order.status), order.note || '', new Date(order.created_at).toLocaleString('vi-VN')])]; const blob = new Blob(['\ufeff' + rows.map(row => row.map(quote).join(',')).join('\n')], { type: 'text/csv;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `qmstore-don-hang-${new Date().toISOString().slice(0, 10)}.csv`; link.click(); URL.revokeObjectURL(url) }
  return <section className="page"><div className="admin-page-head"><div><h1>Báo cáo doanh thu</h1><p className="sub">Tổng hợp từ toàn bộ đơn hàng và các yêu cầu nạp đã được duyệt.</p></div><button className="small-btn" onClick={exportOrders}>Xuất CSV đơn hàng</button></div><div className="metrics report-metrics"><Metric label="Tổng đơn hàng" value={orders.length} hint="Tất cả khách hàng"/><Metric label="Doanh thu ghi nhận" value={money(revenue)} hint="Không gồm hoàn/trượt"/><Metric label="Đã hoàn thành" value={completed} hint="Đơn hoàn tất"/><Metric label="Tiền nạp đã duyệt" value={money(topupTotal)} hint={`${approvedTopups.length} yêu cầu`}/></div><div className="table-panel"><table><thead><tr><th>Trạng thái</th><th>Số đơn</th><th>Giá trị đơn</th></tr></thead><tbody>{['pending', 'processing', 'completed', 'failed', 'refunded'].map(status => { const list = orders.filter(order => order.status === status); return <tr key={status}><td><span className={'status ' + status}>{statusLabel(status)}</span></td><td>{list.length}</td><td>{money(list.reduce((sum, order) => sum + Number(order.total_amount || 0), 0))}</td></tr> })}</tbody></table></div></section>
}
